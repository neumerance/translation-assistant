...
set :application, 'stg-ate-editor'
set :repo_url, 'put_repo_url_here'

set :deploy_to, '~/rails_apps/stg-ate-editor'

set :branch, 'release'

set :rvm_ruby_version, '2.3.1@stg-ate-editor'
...
namespace :deploy do
  ...
  task :update_config_file do
    on roles(:app) do
      execute "git archive --remote=ssh://git_url_here/icl-upgrade/icl-secrets.git HEAD webta/ate-staging/application.yml | tar -xO > #{deploy_to}/shared/config/application.yml"
    end
  end
